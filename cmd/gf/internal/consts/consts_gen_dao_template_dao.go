...
func (dao *{TplTableNameCamelCase}Dao) Group(ctx context.Context) string {
	group := ctx.Value("{TplDynamicGroup}").(string)
	if group != "" {
		 return group
	}
	return dao.group
...
func (dao *{TplTableNameCamelCase}Dao) Ctx(ctx context.Context) *gdb.Model {
	schema := ctx.Value("{TplDynamicSchema}").(string)
	return g.DB(dao.Group(ctx)).Schema(schema).Model(dao.table).Safe().Ctx(ctx)
}
